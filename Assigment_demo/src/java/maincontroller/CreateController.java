...
public class CreateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (productID.length() < 1 || productID.length() > 7) {
                productError.setProductIDError("ProductId must be in [1, 7]");
                check = false;
            }
            if (productName.length() < 1 || productID.length() > 20) {
                productError.setProductIDError("ProductName must be in [1, 20]");
                check = false;
            }
            if(check){
